...
% Logo Superior (Reemplazar 'logo_superior.png' con el archivo real)

% --- Bloque Superior (Vicerrectoría) ---
\noindent % Evita la sangría del párrafo
\begin{minipage}[c]{0.2\textwidth} % Minipágina para el logo
    \centering
    % Descomenta y usa tu logo real.
    \includegraphics[width=2cm]{logo_ufro.png} 
\end{minipage}% <-- El % es crucial para evitar espacios indeseados
\hspace{0.1cm} % Espacio reducido entre logo y texto
\begin{minipage}[c]{0.7\textwidth} % Minipágina para el texto
    \large
    \textbf{UNIVERSIDAD DE LA FRONTERA}\\[0.1cm] % Separación reducida
    \textsc{VICERRECTORÍA DE INVESTIGACIÓN Y POSTGRADO}
\end{minipage}

\centering
% Logo Central (Reemplazar 'logo_central.png' con el archivo real)
\includegraphics[width=5cm]{logo_ufro.png}\\[2cm]
...
\textbf{N° DE MATRÍCULA:} 16575262725\\ % Reemplazar con la matrícula real
\textbf{PROGRAMA:} MII % Reemplazar si es necesario
\end{flushleft}
...
\tableofcontents
\newpage
\pagenumbering{arabic} % Volver a numeración arábiga para el contenido
\setcounter{page}{1} % Reiniciar el contador de página
% --- FIN DEL ÍNDICE ---


% --- INICIO DEL CONTENIDO DEL PAPER ---
\maketitle
